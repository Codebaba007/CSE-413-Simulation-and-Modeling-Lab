# CSE 413 - Simulation and Modeling Lab

This repository contains my Simulation and Modeling Lab assignments and practical work.

## Course Information

- **Course:** CSE 413 - Simulation and Modeling Lab
- **Student:** Mehedi Hassan Bhuiyan
- **Student ID:** 1080
- **University:** University of Information Technology and Sciences (UITS)

## Lab Files

- [Lab 1 - Scalar, Array & Matrix Operations](SIM_Lab1_MehediHassanBhuiyan_1080.ipynb)
- [Lab 2 - Unique Matrix Value Investigation](SIM_Lab2_MehediHassanBhuiyan_1080.ipynb)

## Topics Covered

### Lab 1
- NumPy Scalar Operations
- Array Operations
- Matrix Operations
- Trigonometric Functions
- Exponential & Logarithmic Functions
- Absolute Value & Square Root
- Remainder Operations
- Rounding Operations
- Vector Functions
- Statistical Operations
- Matrix Visualization

### Lab 2
- Student-specific Matrix Generation
- Matrix Shape
- Determinant
- Matrix Rank
- Eigenvalues
- Matrix Inverse
- Matrix Value Change Experiment
- Comparison of Matrix Properties

## Technologies

- Python
- NumPy
- Jupyter Notebook
